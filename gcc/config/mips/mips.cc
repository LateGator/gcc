...
mips_const_insns (rtx x)
{
  ...
  switch (GET_CODE (x))
      ...
      else
	{
	  ...
	  return mips_build_integer (codes, l[0] | (((HOST_WIDE_INT) l[1]) << 32)) + 1;
	}
...
mips_legitimize_const_move (machine_mode mode, rtx dest, rtx src)
{
  ...
  if (splittable_const_double_operand (src, mode))
      ...
      else
	{
	  ...
	  mips_move_integer (tmp, tmp, l[0] | (((HOST_WIDE_INT) l[1]) << 32));
	}
...
mips_emit_4300_stack_cache_op (HOST_WIDE_INT size, long op, unsigned tmpreg)
{
  ...
  if (size <= linesize * 4)
	...
	emit_insn (gen_mips_cache (GEN_INT (op), plus_constant (Pmode, stack_pointer_rtx,
								-offset)));
    }
  else
    {
	...
	size = MIN (size, (HOST_WIDE_INT) mips_dcache_size);
      ...
      emit_insn (gen_mips_cache (GEN_INT (op), plus_constant (Pmode, tr, -linesize)));
